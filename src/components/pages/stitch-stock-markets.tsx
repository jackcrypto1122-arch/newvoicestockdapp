...
import { STOCK_TOKEN_ADDRESSES } from "@/lib/constants";
...
export function StitchStockMarketsPage() {
    ...
    <StitchShell>
      <div className="p-6 pt-0 pb-8 grid grid-cols-12 gap-4">
        <div className="col-span-12 grid grid-cols-1 gap-4 sm:grid-cols-3">
          <StatCard
            icon="show_chart"
            label="Tracked Stock Assets"
            value={String(markets.length || STOCK_TOKEN_ADDRESSES.length)}
            change="Tokens"
            positive={true}
            plainChange={true}
        ...
        <div className="col-span-12 flex flex-col gap-4 mt-2">
          <GlassPanel>
            <SectionHeader title="Stock Markets Board" />
